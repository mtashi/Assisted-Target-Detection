...
from training_features_labels_diff_win import training_features_labels
from test_features_labels_diff_win import test_features_labels
from rec_prec_fp_rate import rec_prec_fp_rate
from save_TP_diff_win import save_TP
from save_FN_diff_win import save_FN
...
addres_result = '/media/maryam/Data/training_data_NN_HOG/April21st/diff_win_size/HOG/'+str(win_size)+'_all/'


# train
[positive_feature_data, negative_feature_data,
 positive_labels, negative_labels] = training_features_labels(win_size, c, o, b, 480, 640)
...
[Test_Data_Air, Test_Data_noAir, Test_labels_Air,
 Test_labels_noAir, decision_patch_val_true_lable_patch] = test_features_labels(trained_SVM, win_size, c, o, b, 480, 640)
...
# addrs_txt = '/media/maryam/Data/training_data_NN_HOG/April21st/diff_win_size/HOG/'+str(win_size)+'/'
    ...
    if img_label == 'p':
        ...
        for j_h in range(int((img_height-win_size)/step_size)):
            ...
            for j_v in range(int(10/step_size),int((img_width-win_size)/step_size)-int(10/step_size)):
                ...
                img_crop = cv2.resize(img_crop,(win ,win), interpolation = cv2.INTER_AREA)
...
#                     # saving TP and FN
#                     addrs_TP = addres_result + 'results/tp/'
#                     save_TP (addrs_TP,win_size, response[0], decision_value[0],j_h,j_v,
#                              img, img_crop ,step_size, c,o,b)

#                     addrs_FN = addres_result + 'results/fn/'
#                     save_FN (addrs_FN,win_size, response[0], decision_value[0],j_h,j_v,
#                              img, img_crop ,step_size, c,o,b)
...
#         results_addrs = addres_result + 'results/results_'
...
#                 cv2.imwrite( addrs_FP + str(decision_value)+'j_h='+str(j_h)+'j_v='+str(j_v)+'.jpg',hogImage)

#             #else:

#                 addrs_TN =addres_result + 'results/tn/'
#                 cv2.imwrite( addrs_TN + str(decision_value)+'j_h='+str(j_h)+'j_v='+str(j_v)+'.jpg',hogImage)
