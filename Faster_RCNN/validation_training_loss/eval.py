...
from config.train_config import TrainConfig as Config_train
...
#to get training loss and eval loss 
#def _eval(path_to_checkpoint: str, dataset_name: str, backbone_name: str, path_to_data_dir: str, path_to_results_dir: str,num_epoch):
    ...
    model = Model(backbone, dataset.num_classes(), pooler_mode=Config.POOLER_MODE,
                  ...
                  rpn_pre_nms_top_n=Config.RPN_PRE_NMS_TOP_N, rpn_post_nms_top_n=Config.RPN_POST_NMS_TOP_N,anchor_smooth_l1_loss_beta=Config_train.ANCHOR_SMOOTH_L1_LOSS_BETA,
                  proposal_smooth_l1_loss_beta=Config_train.PROPOSAL_SMOOTH_L1_LOSS_BETA).cuda()
    model.load(path_to_checkpoint)

    Log.i('Start evaluating with 1 GPU (1 batch per GPU)')
#original command
    mean_ap, detail= evaluator.evaluate(model)
#to get training loss and eval loss in validation_erro_oss
    #mean_ap, detail, avg_loss_eval = evaluator.evaluate(model)
    Log.i('Done')
#to get training loss and eval loss
    #with open('/home/maryam/easy-faster-rcnn_pr_curve_no_pretrained.pytorch/outputs/m_output/eval_loss/eval_avg_loss.txt', "a") as text_file:

        
        #text_file.write(str(avg_loss_eval)+'\n')

    Log.i('mean AP = {:.4f}'.format(mean_ap))
    Log.i('\n' + detail)
#to get training loss and eval loss
    #Log.i('loss = {:.4f}'.format(avg_loss_eval))
    #Log.i('\n' + detail)
...
# to get the validation training loss 
        #_eval(path_to_checkpoint, dataset_name, backbone_name, path_to_data_dir, path_to_results_dir,num_epoch)
