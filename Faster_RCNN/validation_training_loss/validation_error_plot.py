...
def _train(dataset_name: str, backbone_name: str, path_to_data_dir: str, path_to_checkpoints_dir: str, path_to_resuming_checkpoint: Optional[str],path_to_results_dir: str):
    ...
    while not should_stop:
        for _, (_, image_batch, _, bboxes_batch, labels_batch) in enumerate(dataloader):
            ...
            #print('image_batch_train',image_batch)
            #print('labels_batch_train',labels_batch)
            #print('bboxes_batch_train',bboxes_batch)
            ...
            #print('anchor_objectness_losses:',anchor_objectness_losses)
            #print('anchor_transformer_losses:',anchor_transformer_losses)
            #print('proposal_class_losses:',proposal_class_losses)
            #print('proposal_transformer_losses:',proposal_transformer_losses)
            ...
            if step % 2 == 0:
               ...
               Log.i(f'Model has been saved to , ready for evaluation {path_to_checkpoint}')

               eval._eval(path_to_checkpoint, 'NRCAirplanes', 'resnet18', './data', path_to_results_dir,step)
               
               with open('/home/maryam/easy-faster-rcnn_pr_curve_no_pretrained.pytorch/outputs/m_output/avg_loss/train_avg_loss.txt', "a") as text_file:
   
               		text_file.write(str(avg_loss)+'\n')
...
if __name__ == '__main__':
    def main():
        ...
        parser.add_argument('-o', '--outputs_dir', type=str, default='./outputs/m_output', help='path to outputs directory')
        ...
        #path_to_checkpoints_dir = os.path.join(path_to_outputs_dir, 'checkpoints-{:s}-{:s}-{:s}'.format(time.strftime('%Y%m%d%H%M%S'), dataset_name, backbone_name))
        ...
        print('path_to_checkpoints_dir:',path_to_checkpoints_dir)
        ...
        path_to_results_dir = os.path.join(os.path.dirname(path_to_checkpoints_dir), 'results-{:s}-{:s}-{:s}'.format(time.strftime('%Y%m%d%H%M%S'), path_to_checkpoints_dir.split(os.path.sep)[-1].split(os.path.curdir)[0],str(uuid.uuid4()).split('-')[0]))
        ...
        _train(dataset_name, backbone_name, path_to_data_dir, path_to_checkpoints_dir, path_to_resuming_checkpoint,path_to_results_dir)
    main()
